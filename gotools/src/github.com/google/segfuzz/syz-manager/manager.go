...
func (mgr *Manager) loadInterleavingCoverage() {
	...
	if err != nil {
		log.Logf(0, "[WARN] Failed to read interleaving coverage")
	} else {
...
func (mgr *Manager) dumpCoverage() {
	...
	instCount := mgr.serializeInstCount()
	instBlacklist := mgr.serializeBlacklist()
...
func (mgr *Manager) serializeInstCount() bytes.Buffer {
	var instCount bytes.Buffer
	mgr.serv.mu.Lock()
	defer mgr.serv.mu.Unlock()
...
func (mgr *Manager) serializeBlacklist() bytes.Buffer {
	var instBlacklist bytes.Buffer
	mgr.serv.mu.Lock()
	defer mgr.serv.mu.Unlock()
	for k := range mgr.serv.instBlacklist {
		instBlacklist.WriteString(fmt.Sprintf("%x\n", k))
	}
	return instBlacklist
}
...
// TODO: load/writeBlacklist() and dumpCoverage() is somewhat
// redundant
func (mgr *Manager) loadBlacklist() {
	fn := filepath.Join(mgr.cfg.Workdir, fmt.Sprintf("blacklist-%v", hex.EncodeToString(mgr.kernelHash)))
	data, err := ioutil.ReadFile(fn)

	if err != nil {
		log.Logf(0, "[WARN] Failed to read blacklist")
	} else {
		str := string(data)
		for _, s := range strings.Split(str, "\n") {
			...
			n := uint32(n64)
			mgr.serv.instBlacklist[n] = struct{}{}
			mgr.stats.instBlacklist.set(len(mgr.serv.instBlacklist))
		}
	}
...
func (mgr *Manager) writeBlacklist(blacklist map[uint32]struct{}) {
	for inst := range blacklist {
		if _, err := mgr.blacklistFile.WriteString(fmt.Sprintf("%x\n", inst)); err != nil {
			panic(err)
		}
	}
}
